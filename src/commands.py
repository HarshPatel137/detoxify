...
def _embed_thr(gid: str, cid: str, title: str = "Toxicity policy") -> discord.Embed:
...
class ThresholdModal(discord.ui.Modal):
    """Modal to set a custom threshold for one label."""
    def __init__(self, cb, label_key: str):
        super().__init__(title="Set custom threshold (0..1)")
        self.cb = cb
        self.label_key = label_key
        self.value_input = discord.ui.TextInput(
            label=NICE.get(label_key, label_key),
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("Enter a number between 0 and 1.", ephemeral=True)
            return
        v = max(0.0, min(1.0, v))
        await self.cb(interaction, self.label_key, v)

    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        try:
            await interaction.response.send_message("Something went wrong saving that value.", ephemeral=True)
...
class PolicySetView(discord.ui.View):
    ...
    class CustomBtn(discord.ui.Button):
        ...
        async def callback(self, inter: discord.Interaction):
            ...
            async def done(i: discord.Interaction, label: str, thr: float):
...
class ToxicityCommands(app_commands.Group):
    ...
    async def status(self, inter: discord.Interaction):
        ...
        e = discord.Embed(
            title="Your Respect Score",
            color=0x10B981 if s >= 75 else 0xF59E0B if s >= 60 else 0xEF4444,
        )
        ...
        await inter.response.send_message(
            "Here’s your 7-day snapshot (CSV attached).",
            embed=e,
            file=file,
            ephemeral=True,
        )
